...
const ProductList = () => {
  // State untuk produk menggunakan Map
  const [productsMap, setProductsMap] = useState(() => {
    const map = new Map();
    initialProducts.forEach((product) => map.set(product.id, product));
    return map;
  });
  ...
  return (
    <div className="flex gap-4 p-8">
      {[...productsMap.values()].map((product) => (
        <Card
          key={product.id}
          product={product}
